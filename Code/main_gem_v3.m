...
figure('Name', '开环系统分析'); step(sys_ol); title('开环系统阶跃响应'); grid on;
...
figure('Name', '理想控制器性能对比'); plot(t_pp, y_pp, 'b-'); hold on; plot(t_lqr, y_lqr, 'r--'); 
title('理想全状态反馈：极点配置法 vs LQR法'); legend('极点配置法', 'LQR法'); grid on; hold off;
...
figure('Name', '最终性能对比：理想 vs 观测器'); plot(t_pp, y_pp, 'b-'); hold on; plot(t_aug, y_aug, 'g--');
title('系统输出对比'); legend('理想全状态反馈', '基于观测器的反馈'); grid on; hold off;
t = t_lqr; u = ones(size(t)); [~, ~, x_aug] = lsim(sys_aug, u, t);
x_real = x_aug(:, 1:n); e = x_aug(:, n+1:end); x_hat = x_real - e;
figure('Name', '状态估计效果分析'); plot(t, x_real(:,2), 'b-'); hold on; plot(t, x_hat(:,2), 'm:');
title('状态估计效果：角速度 (\omega)'); legend('真实角速度', '估计角速度'); grid on; hold off;
